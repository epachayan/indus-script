...
raw=[r for r in csv.DictReader(open('../data/indus_decipher/data/indus_website_real_corpus.csv')) if grp(r['motif'])]
seen=set(); rs=[]
for r in raw:
    k=(grp(r['motif']),r['sign_sequence'])
    if k not in seen: seen.add(k); rs.append(r)
print('dedup',len(raw),'->',len(rs))
...
json.dump(hits,open('motif_hits_dedup.json','w'))
